    ...
    private void Update()
    {
        if (!this.disable)
        {
            ...
            if ((Vector3.Distance(this.desPt, base.transform.position) < 20f) || (base.transform.position.y < 0f))
            {
                ...
                {
                    obj2 = (GameObject) Network.Instantiate(Resources.Load("FX/boom1_CT_KICK"), base.transform.position + ((Vector3) (Vector3.up * 30f)), Quaternion.Euler(270f, 0f, 0f), 0);
                }
                else
                {
                    obj2 = (GameObject) UnityEngine.Object.Instantiate(Resources.Load("FX/boom1_CT_KICK"), base.transform.position + ((Vector3) (Vector3.up * 30f)), Quaternion.Euler(270f, 0f, 0f));
                }
                this.disable = true;
            }
        }
