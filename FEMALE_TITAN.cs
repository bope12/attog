...
{
    [CompilerGenerated]
    private static Dictionary<string, int> SWITCHSMAP0;
    [CompilerGenerated]
    private static Dictionary<string, int> SWITCHSMAP1;
    [CompilerGenerated]
    private static Dictionary<string, int> SWITCHSMAP2;
    ...
    private void attack(string type)
    {
        ...
        if (key != null)
        {
            ...
            if (SWITCHSMAP1 == null)
            {
                ...
                SWITCHSMAP1 = dictionary;
            }
            if (SWITCHSMAP1.TryGetValue(key, out num))
            {
                switch (num)
                {
                    case 0:
    ...
    private bool attackTarget(GameObject target)
    {
        ...
        if (key != null)
        {
            if (SWITCHSMAP0 == null)
            {
                ...
                SWITCHSMAP0 = dictionary;
            }
            if (SWITCHSMAP0.TryGetValue(key, out num5))
            {
                switch (num5)
                {
                    case 0:
    ...
    private GameObject checkIfHitHead(Transform head, float rad)
    {
        ...
        {
            if ((obj2.GetComponent<TITAN_EREN>() == null) && !gameObject.GetComponent<HERO>().isInvincible())
            {
    ...
    private void grab(string type)
    {
        ...
        if (key != null)
        {
            ...
            if (SWITCHSMAP2 == null)
            {
                ...
                SWITCHSMAP2 = dictionary;
            }
            if (SWITCHSMAP2.TryGetValue(key, out num))
            {
                switch (num)
                {
                    case 0:
    ...
    [RPC]
    public void hitAnkleLRPC(NetworkViewID player, int dmg)
    {
        if (!this.hasDie && (this.state != "anklehurt"))
        {
            ...
            Vector3 vector = NetworkView.Find(player).gameObject.transform.position - base.transform.Find("Amarture/Core/Controller_Body").transform.position;
            if (vector.magnitude < 20f)
            {
                ...
                if (player.isMine)
                {
                    GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().FTankleGetHurtbyServer(dmg, base.name);
                }
                else
                {
                    GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().FTankleGetHurt(player, dmg, base.name);
                }
            }
        }
    }
    ...
    [RPC]
    public void hitAnkleRRPC(NetworkViewID player, int dmg)
    {
        if (!this.hasDie && (this.state != "anklehurt"))
        {
            if ((Network.peerType == NetworkPeerType.Server) && (this.grabbedTarget != null))
    ...
    private void Start()
    {
        ...
        MonoBehaviour.print(string.Concat(new object[] { "female HP:", this.NapeArmor, "/", this.AnkleLHPMAX }));
        if (FengMultiplayerScript.isAnnieasymodo)
        {
            this.NapeArmor = 10;
            CapsuleCollider[] componentsInChildren = base.gameObject.GetComponentsInChildren<CapsuleCollider>();
            foreach (CapsuleCollider collider in componentsInChildren)
            {
    ...
    public void titanGetHit(NetworkViewID player, int speed)
    {
        if (base.networkView.isMine)
        {
            ...
            if (vector.magnitude < 15f)
            {
                ...
                {
                    GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().FTgetHurtbyServer(speed, base.name);
                }
                else
                {
                    GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().FTgetHurt(player, speed, base.name);
                }
            }
        }
    }
        ...
        if ((!IN_GAME_MAIN_CAMERA.isPausing || (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE)) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.networkView.isMine))
        {
            if (this.hasDie)
            {
                ...
                if (base.animation["die"].normalizedTime >= 1f)
                {
                    ...
                    if (!FengMultiplayerScript.isAnniecalypse)
                    {
                        for (int i = 0; i < 15; i++)
                        {
                            GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().randomSpawnTitan("titanRespawn", 50).GetComponent<TITAN>().beTauntedBy(base.gameObject, 20f);
                        }
                    }
                }
                if ((this.dieTime > 2f) && !this.hasDieSteam)
                {
                    ...
                    {
                        GameObject obj4 = (GameObject) Network.Instantiate(Resources.Load("FX/FXtitanDie1"), base.transform.Find("Amarture/Core/Controller_Body/hip").position, Quaternion.Euler(-90f, 0f, 0f), 0);
                ...
                if (this.dieTime > 20f)
                {
                    ...
                    {
                        GameObject obj6 = (GameObject) Network.Instantiate(Resources.Load("FX/FXtitanDie"), base.transform.Find("Amarture/Core/Controller_Body/hip").position, Quaternion.Euler(-90f, 0f, 0f), 0);
                        obj6.transform.localScale = base.transform.localScale;
            ...
            else
            {
                ...
                else if (this.state == "attack")
                {
                    if ((!this.attacked && (this.attackCheckTime != 0f)) && (base.animation["attack_" + this.attackAnimation].normalizedTime >= this.attackCheckTime))
                    {
                        ...
                        if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SERVER)
                        {
                            obj7 = (GameObject) Network.Instantiate(Resources.Load("FX/" + this.fxName), this.fxPosition, this.fxRotation, 0);
                        }
                        else
                        {
                    ...
                    if ((this.attackCheckTimeA != 0f) && (((base.animation["attack_" + this.attackAnimation].normalizedTime >= this.attackCheckTimeA) && (base.animation["attack_" + this.attackAnimation].normalizedTime <= this.attackCheckTimeB)) || (!this.attackChkOnce && (base.animation["attack_" + this.attackAnimation].normalizedTime >= this.attackCheckTimeA))))
                    {
                        ...
                        foreach (RaycastHit hit in this.checkHitCapsule(this.checkHitCapsuleStart.position, this.checkHitCapsuleEnd.position, this.checkHitCapsuleR))
                        {
                            ...
                            if (gameObject.tag == "erenHitbox")
                            {
                                ...
                                else if ((this.attackAnimation == "combo_3") && (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SERVER))
                                {
