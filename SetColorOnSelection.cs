...
[AddComponentMenu("NGUI/Examples/Set Color on Selection"), RequireComponent(typeof(UIWidget)), ExecuteInEditMode]
public class SetColorOnSelection : MonoBehaviour
{
    [CompilerGenerated]
    private static Dictionary<string, int> SWITCHSMAP3;
    ...
    private void OnSelectionChange(string val)
    {
        ...
        if (key != null)
        {
            ...
            if (SWITCHSMAP3 == null)
            {
                ...
                SWITCHSMAP3 = dictionary;
            }
            if (SWITCHSMAP3.TryGetValue(key, out num))
            {
                switch (num)
                {
                    case 0:
