...
public class DebugConsole : MonoBehaviour
{
  ...
  Vector2 prevMousePos;
    ...
    StringBuilder _chatString = new StringBuilder();
    FPSCounter fps;
    bool dirty;
    ...
    List<Message> _chat = new List<Message>();
    ...
    _fakeDragRect = new Rect(0.0f, 0.0f, _windowRect.width - 32, 24);
        ...
        this.RegisterCommandCallback("sucker", CMDsucker);
        this.RegisterCommandCallback("respawn", CMDrespawn);
        #endif
        ...
        if(_username == string.Empty)
           _username = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().myLastHeroName;
        if(_servername == string.Empty)
           _servername = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().serverName;
        if(_motd == string.Empty)
           _motd = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().motd;
    }
    ...
    public static object Chat(object chat)
    {
        DebugConsole.Instance.LogChat(Message.Log(chat));
        return chat;
    }
    ...
    object CMDClose(params string[] args)
    {
        ...
        object CMDmod(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().viewmod = !GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().viewmod;
        if (GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().viewmod)
            return "Now viewing mod games";
        else
            return "Now viewing normal games";
    }                
    object CMDsay(params string[] args)
    {
        ...
        object CMDserver(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().handlecmd("server", args);

        return "";
    }
    object CMDrefil(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().handlecmd("restock", args);

        return "Restocked";
    }
    object CMDlm(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().handlecmd("lm", args);

        return "LM";
    }
   object CMDsucker(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().handlecmd("sucker", args);

        return "";
    }
    object CMDrespawn(params string[] args)
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().handlecmd("respawn", args);
    ...
    string GetChatString()
    {
        ...
        return _chatString.ToString();
    }

    void BuildChatString()
    {
        _chatString.Length = 0;

        foreach (Message m in _chat)
        {
            _chatString.AppendLine(m.ToString());
        }
    }
    ...
    {
        _rawLogScrollPos = GUI.BeginScrollView(new Rect(25, 35, 250, 345), _rawLogScrollPos, new Rect(0, 0, 300, (GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().numberOfPlayers * 22)));
        int y = 0;
        string str2;
        if (Network.isServer)
        {
            for (int i = 0; i < GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().numberOfPlayers; i++)
            {
                if (GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().players[i] != null)
                {
                    string name = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().players[i].name;
                    while (name.LastIndexOf("[") > 0)
                    {
                        name = name.Remove(name.LastIndexOf("[-"), 3);
                        name = name.Remove(name.LastIndexOf("["), 8);
                    }
                    object[] objArray1 = new object[] { name , " ", GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().players[i].playerIP };
                    str2 = string.Concat(objArray1);
                    GUI.Label(new Rect(64, y, 300, 20), str2);
                    #if Server
                    if (GUI.Button(new Rect(0, y, 36, 20), "Kick"))
                        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().kickPlayer(GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().players[i].id);
                    if (GUI.Button(new Rect(36, y, 28, 20), "Kill"))
                        GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().killPlayer(GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().players[i].id);
                    #endif
                    y = y + 21;
                }
            }
    ...
    void MainWindow(int windowID)
    {
        ...
        if (Network.isServer)
        {
            _username = GUI.TextField(username, _username);
            _servername = GUI.TextField(servername, _servername);
            _motd = GUI.TextField(motd, _motd);

            if (GUI.Button(userRect, "*"))
            {
                if (_username == string.Empty)
                    _username = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().myLastHeroName;
                GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().myLastHeroName = _username;
                GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().name = _username;
            }
            if (GUI.Button(serverRect, "*"))
            {
                if (_servername == string.Empty)
                    _servername = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().serverName;
                MasterServer.RegisterHost("AOTTG", "[" + GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().map + "]" + _servername, string.Empty);
            }
            if (GUI.Button(motdRect, "*"))
            {
                if (_motd == string.Empty && GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().motd != null)
                    _motd = GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().motd;
                GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().motd = _motd;
            }
        }
        if (GUI.Button(restartRect, "Restart Round"))
        {
            if (Network.isServer)
                GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().restartGame();
            else
                GameObject.Find("MultiplayerManager").networkView.RPC("rconcmd", RPCMode.Server, "restart");        
        }
        if (GUI.Button(lavaRect, "Toggle \n Lava"))
        {
            if (Network.isServer)
                ...
                GameObject.Find("MultiplayerManager").networkView.RPC("rconcmd", RPCMode.Server, "lava");        
        }
        if (GUI.Button(crawlerRect, "Toggle \n Crawler"))
        {
         if (Network.isServer)
            GameObject.Find("MultiplayerManager").GetComponent<FengMultiplayerScript>().change("Crawler");
         else
            GameObject.Find("MultiplayerManager").networkView.RPC("rconcmd", RPCMode.Server, "crawler");
        }
        GUI.backgroundColor = Color.clear;
        if (Network.isServer)
        {
            GUI.Label(new Rect(18, 37, 100, 20), "Username");
            GUI.Label(new Rect(18, 81, 100, 20), "Server Name");
            GUI.Label(new Rect(18, 127, 130, 20), "Message of the Day");
        }
        var calcHeight = GUI.skin.textArea.CalcHeight(guiContent, messageLine.width);
